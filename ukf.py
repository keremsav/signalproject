...
class UnscentedKalmanFilter:
    ...
    def update(self, y):
        ...
        dz = Z - z_pred
        Pxz = np.sum(self.Wc[:, None] * (self.X_pred - self.x[:, None].T) * dz[:, None], axis=0)
...
# Örnek sistem fonksiyonları (kullanım için dışarıdan override edilir)
def f_func(x, u):
    J = 0.01
    c = 0.1
    Kv = 0.01
    theta_dot = x[1]
    omega_dot = (Kv * u - c * x[1]) / J
    return x + np.array([theta_dot, omega_dot]) * 0.01

def h_func(x):
    return x[0]  # sadece pozisyon ölçülüyor
